...
def change(value, amount):
    ...
    change = int((amount - value) * 100)
    # We add hundred to get rid of floating points lack of precision
    print(change)
    
    integer_part = int(change/100)
    print(integer_part)

    # Fill the last element of the result by the change integer_part
    result[-1] = integer_part

    decimal_part = change - integer_part * 100 
    print (decimal_part)

    # Let fill the result by the number of coins we should give him

    while decimal_part > 0:
        for i in range(len(available_change) - 2, -1, -1):
            coin = available_change[i]
            if decimal_part - coin >= 0:
                result[i] += 1
                decimal_part -= coin
                break
            else:
